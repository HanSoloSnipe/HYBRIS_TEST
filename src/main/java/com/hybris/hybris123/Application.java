...
public class Application {
    ...
    private static boolean environmentIsOk(){ 
	    	...
	    	if (!dirOk  || !mvnOk || !gitOk ){
	    		System.out.println("One or more prerequisites are not met for running hybris 123 interactive. Please check then re-run: \n"+
	    				"   Called from within hybris123 "+dirOk+"\n"+
	       			"   Mvn found (mvn --version is running) ? "+mvnOk+"\n"+
	       			"   Git found (git --version is running) ? "+gitOk );
	    		return false;
	    	}
    		return true;
    }  
